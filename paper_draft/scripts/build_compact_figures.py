...
def _load_inputs(root: Path, plan: str) -> FigureInputs:
    ...
    return FigureInputs(
        ...
        economics_stratified=_normalize_method_names(_read_csv(economics_root / "economics_stratified_kstar_per_seed.csv")),
        energy_stratified=_normalize_method_names(_read_csv(energy_root / "energy_stratified_kstar_per_seed.csv")),
...
def build_figures(root: Path, plan: str, output_dir: Path, formats: Iterable[str], n_boot: int) -> list[Path]:
    _setup_style()
    inputs = _load_inputs(root, plan)
...
def main() -> None:
    ...
    created = build_figures(root=root, plan=args.plan, output_dir=output_dir, formats=args.formats, n_boot=args.n_boot)
