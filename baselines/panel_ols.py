...
def _lagged_design(panel: Any, max_lag: int) -> np.ndarray:
...
def _flatten_target(panel: Any, max_lag: int) -> np.ndarray:
    return aligned_targets(panel, max_lag).T.reshape(-1)


def _fit_panel_ols(train_panel: Any, max_lag: int, ridge: float = 1e-6) -> np.ndarray:
    x_train = _lagged_design(train_panel, max_lag)
    y_train = _flatten_target(train_panel, max_lag)
...
def add_forecast_calibration(
    ...
    for baseline_name in ("train_mean", "entity_mean", "persistence", "panel_ols"):
...
__all__ = ["add_forecast_calibration", "aligned_targets", "evaluate_panel_forecast_baselines"]